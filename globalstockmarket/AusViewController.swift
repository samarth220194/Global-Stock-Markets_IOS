...
class AusViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,IndicatorInfoProvider {
    ...
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title : "AUSTRALIA")
    }
    ...
    func downloadJsonWithURL() {
        ...
        let task = URLSession.shared.dataTask(with: URL(string: Properties_Utils().stringurl + "Australia")!) { (data, response, error) in
            if error != nil {
    ...
    // MARK: - Table view data source
     ...
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "AusCell", for: indexPath) as! AusCell
        ...
        if cell.Country!.text! == "Australia"{
            cell.countryFlag?.image = UIImage(named : "australia")
        }
        else if cell.Country!.text! == "New Zealand"{
            cell.countryFlag?.image = UIImage(named : "newzealand")
        }
        else{
